...
export function BloodDonationLeaderboard() {
  ...
  const [targetScores, setTargetScores] = useState<number[]>(departmentData.map(() => 0))
    ...
    const fetchData = () => {
      fetch("/api/leaderboardStats")
        ...
        .then((data) => {
          ...
          const newScores = departmentData.map(dept => data[mapping[dept.id]] || 0)
    ...
    <div className="w-full rounded-3xl bg-gradient-to-r from-amber-300 to-amber-200 p-6 shadow-xl border-4 border-amber-600">
      ...
      <div className="space-y-8">
        {departmentData.map((dept, index) => {
          ...
          const widthPercentage = barMax > 0 ? (animatedScores[index] / barMax) * 100 : 0
